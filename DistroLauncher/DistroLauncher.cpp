...
HRESULT InstallDistribution(bool createUser)
{
    ...
    // Execute /nix/var/nix/profiles/system/activate to set system in its initial state
    DWORD exitCode;
    hr = g_wslApi.WslLaunchInteractive(L"/nix/var/nix/profiles/system/activate", false, &exitCode);
    ...
    if (createUser) {
        ...
        std::wstring userName = Helpers::GetUserInput(MSG_ENTER_USERNAME, 32);

        hr = DistributionInfo::FinalizeSystem(userName);
        if (FAILED(hr)) {
            return hr;
        }
